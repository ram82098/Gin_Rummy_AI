...
public class GinRummyGame {
	...
	public int play(PrintWriter pw, StringBuilder sb){
		...
		while (scores[0] < GinRummyUtil.GOAL_SCORE && scores[1] < GinRummyUtil.GOAL_SCORE) { // while game not over
			...
			while (deck.size() > 2) { // while the deck has more than two cards remaining, play round
				...
				sb.append("Player "+currentPlayer);
				sb.append(",");
				...
				if (!(!drawFaceUp && turnsTaken < 2 && faceUpCard == firstFaceUpCard)) { // continue with turn if not initial declined option
					...
					if (playVerbose) {
						...
						sb.append(discardCard.toString());
						sb.append(",");
					}
					...
					if (playVerbose) {
						...
						if (bestMelds.isEmpty()) {
							sb.append(GinRummyUtil.getDeadwoodPoints(unmeldedCards));
							sb.append(",");
						...
						sb.append(hands.get(currentPlayer).toString());
						sb.append("\r\n");
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * @Author Ryan Maresca ****************************************************
		 * 
		 *  - Create PW pathed to the Data directory
		 *  
		 * 
		 */
		
		StringBuilder sb = new StringBuilder();
		PrintWriter pw = new PrintWriter(new File("/Users/marescar/Desktop/Gin_Rummy_AI/Game_Data/Series_1.csv"));
		/*
		 * *************************************************************************
		 */
		sb.append("Player");
		...
		setPlayVerbose(true);
		new GinRummyGame(new SimpleGinRummyPlayer(), new SimpleGinRummyPlayer()).play(pw,sb);
...
//			String[] header = {"\r\n","* Player *--","--* Face-Up *--","--* Payoff *--", "--* Draws *--", "--* Discards *--", "--* Deadwood *----*", "--* Hand *--"};
//			
//			for(String s : header) {
//				sb.append(s);
//			}
			numP1Wins += game.play(pw,sb);
		}
